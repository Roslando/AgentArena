...
export default function App() {
  ...
  useEffect(() => {
    if (isLive) return;
    let cancelled = false;
    fetch("/samples/sample-showcase.jsonl")
  ...
  return (
    <div className="flex h-screen flex-col text-slate-200">
      <TopBar state={state} isLive={isLive} connected={live.connected} />

      <main className="flex flex-1 flex-col items-center justify-start gap-5 overflow-auto px-4 py-5 lg:flex-row lg:items-stretch lg:justify-center lg:gap-8 lg:overflow-hidden lg:px-8">
        {left ? (
